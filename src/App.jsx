...
/** 自己紹介の表の内容 */
  ...
  const works = [
    ...
    [
      ...
      { label: "概要", text:"日本の伝統的なお茶を街中でも気軽に味わえるカフェ「寛茶」を題材にWebサイトを制作しました。こちらはカフェの実在性を高めるためにメニューや場所まで設定を細かく練りました。サイトはPC版のファーストビューを動画にすることで「美味しそう」「尾心地が良さそう」という印象やカフェの世界観がすぐに伝わるようにしています。またメニューをカルーセルにしたり背景の切り替えに視差効果を使用したりと、世界観を保ちつつ飽きさせないような動きを取り入れました。", link: "サイトを見る ＞", path: "/kancha/kancha.html"},
    ...
    [
      ...
      { label: "概要", text:"転職活動時に使用したポートフォリオサイトです。ブランク期間が長い中の応募だったので、スキルよりも人柄などで「一緒に働きたい」と思ってもらえるかが重要だと考え制作しました。そこで自分の働き方のスタンスをRPGの勇者パーティをモチーフにイラストにすることで、キャッチーかつ印象に残りやすくしました。全体的には閲覧のノイズにならないような心地よい動き・余白の取り方を意識してシンプルにまとめています。", link: "サイトを見る ＞", path: "/portforio/index.html" },
      { label: "作業工程", text:"デザイン / コーディング" },
      { label: "使用ツール", text:"Illustrator / Photoshop / Visual Studio Code" },
    ],
    [
